			...
			<div class="container-fluid">
				<div class="row">
					<table class="table">
										...
										<td>
											<div class="btsl ">
												<a href="giohang/cart_update.php?tru&masp=<?php echo $item_cart['MaSP']; ?>&km=<?php echo $item_cart['KM']; ?>&mau=<?php echo $item_cart['Mau']; ?>">
													<button <?php if ($item_cart['SoLuong'] <= 1) {
																echo 'disabled';
															} ?> class="btsl1  text-center btn-outline-success">-</button></a>
												...
												<a href="giohang/cart_update.php?cong&masp=<?php echo $item_cart['MaSP']; ?>&km=<?php echo $item_cart['KM']; ?>&mau=<?php echo $item_cart['Mau']; ?>">
													<button <?php if ($item_cart['SoLuong'] >= 5) {
																echo 'disabled';
															} ?> class="btsl3 text-center btn-outline-success">+</button></a>
											</div>
										</td>
										<td><?php echo number_format($item_cart['DonGia']) . ' đ'; ?></td>
										<td><?php echo number_format($item_cart['KM']) . ' đ'; ?></td>
										<td><?php $tt = 0;
											$tt1 = $item_cart['SoLuong'] * $item_cart['DonGia'] - $item_cart['KM'] * $item_cart['SoLuong'];
											$tt = $tt + $tt1;
						...
						?> <tr>
							<td><?php if (isset($_SESSION['gio_hang'])) {
									echo '<a href="giohang/cart_update.php?xoaall" >xóa tất cả</a>';
								} ?></td>
							<td></td>
							<td></td>
							<td></td>
							<td></td>
							<th style="font-size: 35px;"><?php if (!isset($_SESSION['gio_hang'])) {
																echo '<i class="far fa-frown"></i> Giỏ hàng của bạn rỗng <i class="far fa-frown"></i>';
															} ?></th>
							<th scope="row" colspan="3">
								<?php if (isset($_SESSION['gio_hang'])) {
									echo	'Tổng tiền :  ' . number_format($money) . ' đ';
								} ?>
							</th>
						</tr>
					</table>
				...
				<div class="row">
					...
					<div class="col-md-6"><a style="  text-decoration: none;" href="index.php?view=thanhtoan" title="">
							<button type="submit" class="btn btn-secondary btn-lg btn-block" name="action" <?php if (!isset($_SESSION['gio_hang'])) {
																												echo 'disabled="disabled"';
																											} ?>> Thanh Toán <i class="fas fa-money-check-alt"></i></button></a>
